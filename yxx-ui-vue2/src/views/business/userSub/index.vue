...
<script>
import { listUserSub, getUserSub, addUserSub, updateUserSub, delUserSub } from "@/api/business/userSub"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listUserSub(this.queryParams).then(response => {
        this.userSubList = response.rows
    ...
    handleUpdate(row) {
      ...
      getUserSub(subId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.subId != null) {
            updateUserSub(this.form.subId, this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addUserSub(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('business/userSub/export', {
        ...this.queryParams
      }, `userSub_export_${this.parseTime(new Date())}.xlsx`)
    }
  }
}
</script>
