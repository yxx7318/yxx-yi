  ...
  <div class="app-container">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['business:userTree:edit']"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
          ...
          <el-button
            ...
            v-hasPermi="['business:userTree:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="节点ID" prop="treeId">
          <treeselect v-model="form.treeId" :options="userTreeOptions" :normalizer="normalizer" placeholder="请选择节点ID" />
        </el-form-item>
        <el-form-item label="节点名称" prop="treeName">
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listUserTree(this.queryParams).then(response => {
        this.userTreeList = this.handleTree(response.data, "userId", "treeId")
        this.loading = false
      })
    },
    /** 转换测试树表生成数据结构 */
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.userId != null) {
            updateUserTree(form.value.userId, this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addUserTree(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('business/userTree/export', {
        ...this.queryParams
      }, `userTree_export_${this.parseTime(new Date())}.xlsx`)
    }
  }
}
</script>
