...
集成Spring AI，实现**会话日志、会话记忆、会话历史、文档读取、向量知识库、MCP能力**等功能，让系统有支持AI智能体能力，所有能力不依赖于API请求，可以完全在本地离线使用：

![image-20251217195718727](img/README/image-20251217195718727.png)

MCP能力：

![image-20251203205217656](img/README/image-20251203205217656.png)

### 基本示例

> 基于大模型开发应用的多种方式，大模型应用开发的技术架构主要有四种：

![image-20251217194724871](img/README/image-20251217194724871.png)

演示视频：https://www.bilibili.com/video/BV1cKq5BKEkL

- **`Function Calling`**：发票录入和查询，由AI去解析文档，调用定义好的函数去录入和操作数据库，进一步应用可以通过AI去通过对话的方式去完成手动操作，可以提高企业数据流通效率和优化工作流
- **`RAG`**：AI可以引用录入的知识库内容回答问题，进一步应用依据企业知识库内容去做智能助手降低培训成本或者发掘企业的数据潜力
...
- 优化代码生成器
  - 增加`swagger`注解代码生成支持，能够在生成`CRUD`代码的基础上自动生成`swagger`注解，增加`knif4j`接口文档增加，弥补RuoYi对接口文档支持差的情况
  ...
  - 迭代和弃用`Date`类，支持`LocalDate`和`LocalDateTime`类，并给数据库表增加统一字段`create_by`、`create_time`等字段
  - 优化前端登录界面，可以直接使用`height`属性
...
- 增加**延迟队列**功能，**增加对异步能力的支持**，并通过Redis进行实现
...
- 优化日志模块，**支持不同环境环境日志级别**，控制台输出和日志输出，更加详尽并支持IDEA点击跳转到对应代码行
- 引入`hutool`依赖，增加的`JsonUtils`、`ObjectUtils`、`SerializationUtils`等工具类
- 优化异步线程池，支持函数式编程
- 增加编程式事务支持，可以使用**函数式编程事务**
...
- 优化`springmvc`异常处理页面，统一为`json`响应
- 优化对静态资源的支持，可以在没有Nginx的情况下单jar包代理静态资源请求
